...
use crate::{
    ...
    core_crypto::{
        ...
        ring::{fast_convert_p_over_q, switch_crt_basis, Matrix, MatrixMut, MatrixRef},
    },
...
>(
    ...
    <C as Ciphertext>::Poly: MatrixMut<'a, u64>,
    <C as Ciphertext>::Poly: MatrixRef<'a, u64>,
{
    ...
    // In BFV multiplication algorithm 2. Size of Q basis is assumed to equal P
    // basis
    let q_size = level + 1;
    let p_size = level + 1;
    ...
    let (basisp_rows, basisp_cols) = parameters.basisp_dimension_at_level(level);
    let mut c00_basisp = <C::Poly as Matrix<u64>>::zeros(p_size, ring_size);
